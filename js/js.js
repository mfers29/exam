...
function gestionarXml(dadesXml) {
    ...
    for (numPregunta=0; numPregunta<2; numPregunta++) {
        var tituloInput = xmlDoc.getElementsByTagName("texto")[numPregunta].innerHTML;
        ponerDatosInputHtml(tituloInput,numPregunta);
        respuestaText[numPregunta] = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName("respuesta")[0].innerHTML;
    }
	
    //SELECT
    //Recuperamos el título y las opciones, guardamos la respuesta correcta
    for (numPregunta=2; numPregunta<4; numPregunta++) {
        var tituloSelect = xmlDoc.getElementsByTagName("texto")[numPregunta].innerHTML;
        var opcionesSelect = [];
        var nopt = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('opcion').length;
        for (i = 0; i < nopt; i++) {
            opcionesSelect[i] = xmlDoc.getElementsByTagName("texto")[numPregunta].getElementsByTagName('opcion')[i].innerHTML;
        }
        ponerDatosSelectHtml(tituloSelect, opcionesSelect, numPregunta);
        respuestaSelect[numPregunta] = parseInt(xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName("respuesta")[0].innerHTML);
    }
    
    //SELECT MULTIPLE
    //Recuperamos el título y las opciones, guardamos las respuestas correctas
    for (numPregunta=4; numPregunta<6; numPregunta++){
        var tituloMultiple = xmlDoc.getElementsByTagName("texto")[numPregunta].innerHTML;
        var opcionesMultiple = [];
        var nopt = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('opcion').length;
        for (i = 0; i < nopt; i++) {
            opcionesMultiple[i] = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('opcion')[i].innerHTML;
        }
        ponerDatosMultipleHtml(tituloMultiple, opcionesMultiple, numPregunta);
        var nres = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('respuesta').length;
        respuestasMultiple[numPregunta]=[];
        for (i = 0; i < nres; i++) {
            respuestasMultiple[numPregunta][i] = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName("respuesta")[i].innerHTML;
        }
    }
    ...
    for (numPregunta=6; numPregunta<8; numPregunta++){
        var tituloCheckbox = xmlDoc.getElementsByTagName("texto")[numPregunta].innerHTML;
        var opcionesCheckbox = [];
        var nopt = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('opcion').length;
        for (i = 0; i < nopt; i++) {
            opcionesCheckbox[i] = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('opcion')[i].innerHTML;
        }
        ponerDatosCheckboxHtml(tituloCheckbox, opcionesCheckbox, numPregunta);
        var nres = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('respuesta').length;
        respuestasCheckbox[numPregunta]=[];
        for (i = 0; i < nres; i++) {
            respuestasCheckbox[numPregunta][i] = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName("respuesta")[i].innerHTML;
        }
    }
}
    ...
    for (numPregunta=8 ; numPregunta<10;numPregunta++) {
        var tituloRadio = xmlDoc.getElementsByTagName("texto")[numPregunta].innerHTML;
        var opcionesRadio = [];
        var nopt = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('opcion').length;
        for (i = 0; i < nopt; i++) {
            opcionesRadio[i] = xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName('opcion')[i].innerHTML;
        }
        ponerDatosRadioHtml(tituloRadio, opcionesRadio, numPregunta);
        respuestaRadio[numPregunta] = parseInt(xmlDoc.getElementsByTagName("pregunta")[numPregunta].getElementsByTagName("respuesta")[0].innerHTML);
    }
...
function ponerDatosSelectHtml(t, opt, numPregunta) {
    document.getElementsByTagName("p")[numPregunta].innerHTML = t;
